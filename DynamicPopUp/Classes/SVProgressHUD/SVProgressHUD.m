...
- (void)updateViewHierachy {
    ...
    if(!self.hudView.superview) {
        [APP_DELEGATE.window addSubview:self.hudView];
        self.hudView.layer.zPosition = MAXFLOAT;
    }
}
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
   ...
   // self.hudView.center = CGPointMake(newCenter.x + self.offsetFromCenter.horizontal, newCenter.y + self.offsetFromCenter.vertical);
    ...
    self.hudView.center = APP_DELEGATE.window.center;
    _imgIcon.center = APP_DELEGATE.window.center;
...
- (UIView*)hudView {
    if(!_hudView) {
        ...
        _imgIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"circle-logo.png"]];
        _imgIcon.frame = CGRectMake(0, 0, 46, 46);
        _imgIcon.contentMode = UIViewContentModeScaleAspectFit;
        [self.overlayView addSubview:_imgIcon];
    }
